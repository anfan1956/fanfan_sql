if OBJECT_ID('web.order_details_delivery_json') is not null drop function web.order_details_delivery_json
go 
create function web.order_details_delivery_json(@json varchar(max)) returns varchar(max) as 
begin 
	...
	for json path
		...
		for json path) order_composition for json path)
...
select web.order_details_delivery_json(@json)