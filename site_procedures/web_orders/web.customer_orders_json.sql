...
go
create function web.customer_orders_json (@phone varchar(max)) returns varchar(max) as 
	begin
		...
		select @orders = (select orderid [№ заказа], format([дата заказа], 'dd.MM.yyyy') дата , format([дата заказа], 'HH:MM') время, 
			case [склад получения]
				when 'доставка'  then 'доставка'
				else 'самовывоз' end 'способ доставки',
			format(оплачено, '#,##0 руб') оплачено, 
			'в работе' статус
...
declare @json varchar(max) = '{"phone":"9167834241"}'
