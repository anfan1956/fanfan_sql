...
create function acc.tAccount_ (@account varchar(max), @contractor varchar(max), @date datetime ) returns table as return
	with s (transId, transDate, item, amount, document, showroom, brand, article, category, barcodeid, comment, [register/saleid]) as (
		select 
		...
		union all
		select transactionid, transdate, item, amount, document, r.showroom, r.brand, r.article, r.category, r.barcodeID, comment, [register/saleid]
		from acc.consignmentReport_ (@contractor, @account) r
		where r.transdate>=@date
	)
select * from s
