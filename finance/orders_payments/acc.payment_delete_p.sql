	...
	begin try
		begin transaction
			...
			if @payments_deleted >0 
				select  'transaction # '  + cast (@paymentid as varchar(max)) + ' deleted'
			else 
				select  'transaction # '  + cast (@paymentid as varchar(max)) + '  did not exist'
...
--declare @note varchar(max), @paymentid int = 7858; exec acc.payment_delete_p  @paymentid; 
