...
--		select @register_credit credit, @register_debet debet, @transactionid transactionid;
		
		select @note= 'получатель: ' + @payee + ', сумма: '  + FORMAT(@amount, '#,##0.00') + ' ' + (select c.currencycode  from acc.registers r join cmn.currencies c on c.currencyID= r.currencyid
