...
create function hr.salaryTopBE_date_f (@date date) returns table as return
	with s (personid, entrydate, document, clientid, amount, num) as (
...
--select 
--	v.personid, v.дата, v.документ, v.комментарий, 
--	ROW_NUMBER () over (partition by personid, документ order by дата) num
--from rep.payroll_report_v v 
